...
impl BenchmarkApp {
    fn new(
        ...
        let hud = HudState {
            ...
            decoder_plugin: config.decoder_plugin.clone(),
            active_streams: 0,
...
fn main() -> iced::Result {
    ...
    let zero_copy_caps = zero_copy::ZeroCopyCaps::probe();
    println!("=== 6-Hour RTSP Video Grid Benchmark (Rust Iced GPU Zero-Copy) ===");
    println!("Platform:          {}", zero_copy_caps.platform);
    println!("Machine ID:        {}", config.machine_id);
    println!("Stream Count:      {}", config.stream_count);
    println!("Target RTSP URL:   {}", config.rtsp_url);
    println!("Target Resolution: {}x{} @ {} FPS", config.video_width, config.video_height, config.target_fps);
    println!("Hardware Decoder:  {} (Native: {})", config.decoder_plugin, zero_copy_caps.hardware_decoder);
    println!("Rendering Backend: iced_wgpu ({})", zero_copy_caps.native_backend);
    println!("NV12 GPU Shaders:  Direct BT.709 Hardware Color Conversion (5.5 MB vs 14.7 MB)");
    #[cfg(target_os = "macos")]
    zero_copy::macos::init_metal_zero_copy();
    #[cfg(target_os = "windows")]
    zero_copy::windows::init_dxgi_zero_copy();
    #[cfg(target_os = "linux")]
    zero_copy::linux::init_linux_zero_copy();
    ...
    let stream_manager = Arc::new(StreamManager::new(
        ...
        config.decoder_plugin.clone(),
        config.video_width,
        config.video_height,
    ));

    println!("[*] Launching {} GStreamer hardware decoders ({})...", config.stream_count, config.decoder_plugin);
    ...
    iced::application(
        move || BenchmarkApp::new(app_config.clone(), app_stream_mgr.clone(), app_telemetry.clone()),
        BenchmarkApp::update,
        BenchmarkApp::view,
    )
